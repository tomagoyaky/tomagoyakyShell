...
import com.tomagoyaky.parseresource.type.ResChunkHeader;
import com.tomagoyaky.parseresource.type.ResStringPoolHeader;
import com.tomagoyaky.parseresource.type.ResStringPoolRef;
import com.tomagoyaky.parseresource.type.ResTableConfig;
import com.tomagoyaky.parseresource.type.ResTableEntry;
import com.tomagoyaky.parseresource.type.ResTableHeader;
import com.tomagoyaky.parseresource.type.ResTableMap;
import com.tomagoyaky.parseresource.type.ResTableMapEntry;
import com.tomagoyaky.parseresource.type.ResTablePackage;
import com.tomagoyaky.parseresource.type.ResTableRef;
...
public class ParseResourceUtils {
	...
	public static int resStringPoolChunkOffset;// 字符串池的偏移值
	public static int packageChunkOffset;// 包内容的偏移值
	public static int keyStringPoolChunkOffset;// key字符串池的偏移值
	public static int typeStringPoolChunkOffset;// 类型字符串池的偏移值

	// 解析资源的类型的偏移值
	public static int resTypeOffset;

	public static ArrayList<String> resStringList = new ArrayList<String>();// 所有的字符串池
	public static ArrayList<String> keyStringList = new ArrayList<String>();// 所有的资源key的值的池
	public static ArrayList<String> typeStringList = new ArrayList<String>();// 所有类型的值的池

	// 资源包的id和类型id
	public static int packId;
	public static int resTypeId;
	public static boolean flag;

	public static void setDebug(boolean _flag) {
		flag = _flag;
	}
	...
	public static void parseResTableHeaderChunk(byte[] src) {
		...
		if (flag)
			Logger.LOGD("header:" + resTableHeader);
	...
	public static void parsePackage(byte[] src) {
		...
		if (flag)
			Logger.LOGD("package size:" + resTabPackage.header.headerSize);
		...
		if (flag)
			Logger.LOGD("keyString:" + resTabPackage.keyStrings);
	...
	public static void parseResTypeSpec(byte[] src) {
		if (flag)
			Logger.LOGD("res type spec offset:"
		...
		if (flag)
			Logger.LOGD("res type spec:" + typeSpec);

		if (flag)
			Logger.LOGD("type_name:" + typeStringList.get(typeSpec.id - 1));
	...
	public static void parseResTypeInfo(byte[] src) {
		...
		resConfig = parseResTableConfig(Utils.copyByte(src, offset + 12,
				resConfig.getSize()));
		if (flag)
			Logger.LOGD("config:" + resConfig);

		if (flag)
			Logger.LOGD("res type info:" + type);

		if (flag)
			Logger.LOGD("type_name:" + typeStringList.get(type.id - 1));

		// 先获取entryCount个int数组
		if (flag)
			Logger.LOGD("type int elements:");
		...
		for (int i = 0; i < type.entryCount; i++) {
			...
			if (entry.flags == 1) {
				...
				mapEntry = parseResMapEntry(Utils.copyByte(src, valueOffset,
						mapEntry.getSize()));
				if (flag)
					Logger.LOGD("map entry:" + mapEntry);
	 ...
	 * @return
	 */
	public static ResStringPoolHeader parseStringPoolChunk(byte[] src,
		...
		if (flag)
			Logger.LOGD("header size:" + stringPoolHeader.header.headerSize);
		if (flag)
			Logger.LOGD("size:" + stringPoolHeader.header.size);
		...
		if (flag)
			Logger.LOGD("flag:" + Utils.bytesToHexString(flagByte));
		...
		if (flag)
			Logger.LOGD("string start:"
					+ Utils.bytesToHexString(stringStartByte));

		// 样式内容的开始位置
		byte[] sytleStartByte = Utils.copyByte(src, offset + 16, 4);
		stringPoolHeader.stylesStart = Utils.byte2int(sytleStartByte);
		if (flag)
			Logger.LOGD("style start:" + Utils.bytesToHexString(sytleStartByte));
		...
		if (flag)
			Logger.LOGD("string count:" + stringPoolHeader.stringCount);
		if (flag)
			Logger.LOGD("style count:" + stringPoolHeader.styleCount);
		...
		while (index < stringPoolHeader.stringCount) {
			...
			if (stringSize != 0) {
				...
				try {
					...
					if (flag)
						Logger.LOGD("string encode error:" + e.toString());
				}
				stringList.add(val);
			} else {
				stringList.add("");
	 ...
	 * @return
	 */
	public static ResChunkHeader parseResChunkHeader(byte[] src, int start) {
